...
if(!global.data.allThreadID.includes(threadID) && !ADMINBOT.includes(senderID) && adminbot.adminPaseOnly == true)return api.sendMessage("[ MODE ] - Chỉ admin mới được sử dụng bot trong chat riêng.", threadID, messageID)
    ...
    if (usages[senderID].usages <= 0 && !["daily","check","setluot"].includes(commandName)) return api.sendMessage("Bạn đã hết lượt sử dụng bot trong hôm nay!\ bấm daily để nhận thêm 20 lượt dùng bot", threadID, messageID);
    if (!command) {
      var allCommandName = [];
